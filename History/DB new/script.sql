...
ALTER TABLE User ADD COLUMN role ENUM ('Farmer' , 'Field Officer' , 'Warehouse Officer' , 'Deputy Officer');
...
CREATE TABLE SeasonalDetails(
    ...
    paddy_status ENUM ('Completed' , 'Ongoing'),
    verification_status ENUM ('Pending' , 'Verified' , 'Refuted'),
    note TEXT NULL,
    FOREIGN KEY (FOID) REFERENCES FieldOfficer (FOID) ON DELETE CASCADE,
    FOREIGN KEY (LID) REFERENCES Land (LID) ON DELETE CASCADE
    );

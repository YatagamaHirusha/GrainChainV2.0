...
/*
    ...
    $sql_check_user = "SELECT * FROM Username WHERE username = '$username'";
...
*/
    ...
    $password = $_POST['password'];
    $sql_user = "INSERT INTO user (full_name, email, gender, age, address, contact_number1, contact_number2, username, password, register_date, role, nic_photo) VALUES ('$full_name', '$email', '$gender', '$age', '$address', '$contact_number1', '$contact_number2', '$username', '$password', CURDATE(), '$role', '$id_proof')";
    ...
    if ($role === 'Farmer') {
        ...
        $find_fo_sql = "SELECT FOID FROM FieldOfficer WHERE assign_division = '$division' LIMIT 1";
        $res_fo = mysqli_query($con, $find_fo_sql);

        if (mysqli_num_rows($res_fo) > 0) {
            $row = mysqli_fetch_assoc($res_fo);
            $foid = $row['FOID'];
        } else {
            $foid = "NULL"; // or handle error
        }
        ...
        if ($success) {
            ...
            $sql_farmer = "INSERT INTO farmer (FID, years_of_experience, nic) VALUES ('$uid', '$years_of_experience', '$nic')";
            $success = mysqli_query($con, $sql_farmer);

            if ($success){
                $sql_land = "INSERT INTO land (FID, FOID, land_reg_no, district, agricultural_service_area, location, land_type, irrigation_type, land_size_in_acres, is_verify, division) VALUES ('$uid' , '$foid', '$land_reg_no' , '$district' , '$agricultural_service_area' , '$location' , '$land_type' , '$irrigation_type' , '$land_size_in_acres' , false , '$division')";
    ...
    elseif ($role === 'Field Officer') {
        ...
        if ($success) {
            ...
            if ($result_get_doid && mysqli_num_rows($result_get_doid) > 0) {
                ...
                $sql_fieldofficer = "INSERT INTO FieldOfficer (FOID, assign_division, assign_district, sub_division, DOID, nic)
                                     VALUES ('$uid', '$assign_division', '$assign_district', '$sub_division', '$DOID', '$nic')";
    ...
    elseif ($role === 'Warehouse Officer') {
        ...
        if ($success) {
            ...
            if($result_get_doid && mysqli_num_rows($result_get_doid) > 0 && $result_get_WID && mysqli_num_rows($result_get_WID) > 0){
                ...
                $sql_warehouseofficer = "INSERT INTO warehouseofficer (WOID, assign_district, assign_division, DOID, WID, nic)
                                         VALUES ('$uid', '$assign_district', '$assign_division', '$DOID', '$WID', '$nic')";
